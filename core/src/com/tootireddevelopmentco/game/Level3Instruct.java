 ...
 * The Level3Instruct class sets up the instructions for Level 3, Matter March.
...
public class Level3Instruct implements Screen 
{
	private Skin skin;  
	private Table table; 
	...
	public Level3Instruct (final RabbitRun game)
	{
	this.game= game; 
	stage= new Stage (); 
	Gdx.input.setInputProcessor(stage);
	background= new Texture ("background.png");
	instructions= new Texture ("MatterInstructions.png");
	
	skin = new Skin(Gdx.files.internal("clean-crispy-ui.json"), new TextureAtlas("clean-crispy-ui.atlas"));
	table = new Table(skin);
	table.setFillParent(true);
	//make heading
	Label heading = new Label("Matter March Instructions", skin);
	...
	//display
	table.add(heading).spaceBottom (450).row();
	table.add (back); 
